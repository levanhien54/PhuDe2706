...
class VRAMManager:
    def __init__(self, settings: Settings):
        ...
        # service -> list of outstanding reservations, so the same service held by two
        # concurrent acquirers doesn't clobber the accounting (a plain float would overwrite).
        self._allocated: dict[str, list[float]] = {}
    ...
    async def release(self, service: str) -> None:
        async with self._condition:
            holdings = self._allocated.get(service)
            freed = holdings.pop() if holdings else 0.0
            if holdings is not None and not holdings:
                del self._allocated[service]
